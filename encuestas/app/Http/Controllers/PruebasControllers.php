...
class PruebasControllers extends Controller {
	...
	public function guardar_prueba (Request $request) {
		...
		$tipos_pruebas->tiempo=$datos->minutos.".".$datos->segundos;
    ...
    public function guardar_prueba_edicion (Request $request) {
		...
		$tipos_pruebas->tiempo=$request->minutos.".".$request->segundos;
        $tipos_pruebas->save();
		...
		if ($resultado==1) {
			...
			if ($request->id_tipo_prueba!=2) {
				...
				elseif ($id_tipo_prueba==7)
					...
					$link = "<a target='_blank' href='http://".$_SERVER["HTTP_HOST"]."/encuestas/public/aplicar_perfil/".$autorizaciones->id."'>Perfil de Competencia</a>";
				...
				Mail::send('correo.invitacion_iol', ["email"=>$email, "nombres"=>$nombres, "texto_email"=>$texto_correo], function($message) use($datos, $email, $tipo_prueba, $nombres, $asunto_correo)
					{
						$message->to($email, $nombres)
							->subject($asunto_correo)
							->bcc("jeleicy@gmail.com")
							->bcc("feedback@talentskey.com");
					});
				/************************* EMAIL A ENVIAR ***************************/				
				...
				Mail::send('correo.invitacion', ["email"=>$email, "nombres"=>$nombres, "texto_email"=>$texto_email], function($message) use($datos, $email, $tipo_prueba, $nombres)
					{
						$message->to($email, $nombres)
							->subject('Invitacion a prueba '.$tipo_prueba)
							->bcc("jeleicy@gmail.com")
							->bcc("feedback@talentskey.com");
					});
				/************************* EMAIL A ENVIAR ***************************/				
			}
			//echo "nombre_prueba=".$nombre_prueba;	
				
			if ($nombre_prueba=="TCGO")
				$resultado=self::guardar_seuc($request->nombres_1,$request->apellidos_1,$request->email_1,$empresa, $autorizaciones->id);				
	...
	public function guardar_seuc($nombre,$apellido,$email,$empresa, $id_autorizacion) {
		
		//echo "id=".$id_autorizacion;
		...
		if (!empty($data))
			...
			$reg_eval = "INSERT INTO tb_evaluados (id_evaluado,nombres_evaluado, apellidos_evaluado, 
							...
							seccion_evaluado, id_tutor, nombre_com_cliente, status_evaluado, id_autorizacion) 
						VALUES ($maximo, '$nombre', '$apellido', '$email', 
							'', '', '', 'Cohorte I', 
							'10339892', 'Cliente Tester', 'Activo', ".$id_autorizacion.")";
			//echo "<br><br>".$reg_eval;
			DB::insert($reg_eval);
			...
			$sql_invitacion = "INSERT INTO tb_evaluaciones (cod_evaluacion, id_evaluado, nombres_evaluado, 
							...
							VALUES('$cod_invitacion', '0', '$nombre', '$apellido', '$email', 
								...
								'julio@red-talento.com',".$id_autorizacion.")";
			//echo "<br><br>".$sql_invitacion;
