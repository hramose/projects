...
class BateriaControllers extends Controller {
    ...
    public function guardar_bateria_edicion(Request $request) {
		...
		$sql = "update tablas_funciones set tabla='".$datos["funcion"]."' where id_bateria=".$id;
		DB::update($sql);
		
		$sql = "update tablas_opciones set tabla='".$datos["opciones"]."' where id_bateria=".$id;
		DB::update($sql);

		$sql = "update tablas_preguntas set tabla='".$datos["preguntas"]."' where id_bateria=".$id;
		DB::update($sql);
	
		$sql = "update tablas_resultados set tabla='".$datos["resultados"]."' where id_bateria=".$id;
		DB::update($sql);			
    ...
    public function guardar_bateria_nuevo (Request $request) {		
        ...
        if (empty($data)) {
			...
			$sql = "insert into tablas_funciones (id_bateria,tabla) values($id_bateria,'".$datos["funcion"]."')";
			DB::insert($sql);
			
			$sql = "insert into tablas_opciones (id_bateria,tabla) values($id_bateria,'".$datos["opciones"]."')";
			DB::insert($sql);

			$sql = "insert into tablas_preguntas (id_bateria,tabla) values($id_bateria,'".$datos["preguntas"]."')";
			DB::insert($sql);

			$sql = "insert into tablas_resultados (id_bateria,tabla) values($id_bateria,'".$datos["resultados"]."')";
			DB::insert($sql);		
