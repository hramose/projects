...
function startTimer(tiempo) {
	...
	function guardar_encuesta_cc_1 () {
		...
		//if (nro_prueba==2) {
				...
				for (j=0; j<f.length; j++) {
					...
					if (f.elements[j].name.indexOf("coord_")>-1) {
						...
						if (id_pregunta==0 || nombre.substr(6,2)!=id_pregunta) {
							...
							resultado+="&resultado"+index_res+"="+id_pregunta+".";
							index_res++;
							entro=1;
						}
						resultado+=valor+",";
			...
			//document.getElementById("xxx").innerHTML=resultado;
		//}
		//alert("../index_guardar_prueba_coordinacion_1?"+resultado+"&id_au="+id_au+"&proxima_pagina="+proxima_pagina+"&id_bateria="+id_bateria+"&nro_prueba="+nro_prueba);
		location.href="../index_guardar_prueba_coordinacion_1?"+resultado+"&id_au="+id_au+"&proxima_pagina="+proxima_pagina+"&id_bateria="+id_bateria+"&orden="+orden;
	}
	
	function guardar_encuesta_hic() {
		...
		for (i=1; i<f.length; i++) {
			//for (j=0; j<f.length; j++) {
				...
				if (f.elements[i].name.indexOf("opc_hi_")>-1) {
		...
		//alert(resultado_hic);
		location.href="../index_guardar_prueba_intelectivas?resultado="+resultado_hic+"&id_au="+id_au+"&proxima_pagina="+proxima_pagina+"&id_bateria="+id_bateria+"&orden="+orden;
	}
	
	function guardar_encuesta_cd() {
		...
		for (i=1; i<f.length; i++) {
			if (f.elements[i].name.indexOf("op_")>-1) {
				if (f.elements[i].checked==true) {
					valor=f.elements[i].value;
					opcion=f.elements[i].name.substr(3);
		...
		//alert(resultado);
		location.href="../index_guardar_prueba_competencias"+resultado+"&id_au="+id_au+"&proxima_pagina="+proxima_pagina+"&id_bateria="+id_bateria+"&orden="+orden	
	}	
	...
	function guardar_encuesta_iep (tipo) {
		...
		var num=1;
		var resultado_mas="&resultado_mas_"+num+"=";
		var resultado_menos="&resultado_menos_"+num+"=";
		num++;
		...
		*/
		...
		for (i=1; i<respuesta_iep_mas.length; i++) {
			if (typeof respuesta_iep_mas[i] !== 'undefined') {
				if (i%10==0) {
					resultado_mas+="&resultado_mas_"+num+"=";
					num++;
				}				
		...
		for (i=1; i<respuesta_iep_menos.length; i++) {
			if (typeof respuesta_iep_menos[i] !== 'undefined') {
				if (i%10==0) {
					resultado_menos+="&resultado_menos_"+num+"=";
					num++;
				}
		...
		if (tipo==0) {
			...
			location.href="../index_guardar_prueba_iep?id_bateria="+bateria+resultado_mas+resultado_menos+"&id_au="+id_au+"&orden="+orden;
	}
	
	function exportar (id_bateria) {
		//alert(id_bateria);
		var f=eval("document.forms[0]");
		var fi=f.fecha_reporte1.value;
		var ff=f.fecha_reporte2.value;
		
		fi=fi.replace("/","-");
		fi=fi.replace("/","-");
		fi=fi.replace("Enero","-1-");
		fi=fi.replace("Febrero","-2-");
		fi=fi.replace("Marzo","-3-");
		fi=fi.replace("Abril","-4-");
		fi=fi.replace("Mayo","-5-");
		fi=fi.replace("Junio","-6-");
		fi=fi.replace("Julio","-7-");
		fi=fi.replace("Agosto","-8-");
		fi=fi.replace("Septiembre","-9-");
		fi=fi.replace("Octubre","-10-");
		fi=fi.replace("Noviembre","-11-");
		fi=fi.replace("Diciembre","-12-");
		
		ff=ff.replace("/","-");
		ff=ff.replace("/","-");
		ff=ff.replace("Enero","-1-");
		ff=ff.replace("Febrero","-2-");
		ff=ff.replace("Marzo","-3-");
		ff=ff.replace("Abril","-4-");
		ff=ff.replace("Mayo","-5-");
		ff=ff.replace("Junio","-6-");
		ff=ff.replace("Julio","-7-");
		ff=ff.replace("Agosto","-8-");
		ff=ff.replace("Septiembre","-9-");
		ff=ff.replace("Octubre","-10-");
		ff=ff.replace("Noviembre","-11-");
		ff=ff.replace("Diciembre","-12-");
		
		location.href="http://iol.talentskey.net/encuestas/public/exportar_reporte_completo/"+id_bateria+","+fi+","+ff;
	}
	...
	function guardar_encuesta_co(tipo) {
		...
		if (nro_prueba==2) {
			...
			//alert(document.getElementById("instrucciones").style.display);
			/*if (document.getElementById("instrucciones").style.display=="inline") {
				document.getElementById("encuesta").style.display="inline";
				document.getElementById("instrucciones").style.display="none";
			} else {*/
				id_bateria=document.forms[0].bateria.value;
				...
				if (tipo==0) {
					...
					location.href="../prueba_resto/"+id_au+"-"+id_bateria+"-"+orden;
			//}
		}
		return false;
	}
	...
	function acepto() {
		...
		if (URLactual.toString().indexOf("epa")>-1)
			prox_link="../prueba_epa_ejemplo/"+id_au+"-1-"+id_bateria;
		else
			prox_link="../prueba_siguiente/"+id_au+"-1-"+id_bateria;
		//alert(prox_link);
		location.href=prox_link;
	}
	...
	function colocar(opcion,preguntas, id_opcion) {
		if (opcion.indexOf("imagenes")>-1)
			$("#respuesta_"+preguntas).html("<img src='../"+opcion+"' border=0 height='60' width='60'>");
		else
			$("#respuesta_"+preguntas).html("<strong>"+opcion+"</strong>");
		
		/*if (resultado_hic.indexOf(id_opcion+",") > -1)
			x=0;
		else*/
			resultado_hic+=id_opcion+",";
		//alert(resultado_hic);
	}
	...
	function colocar_test(solucion, opcion, valor, prueba, nombre) {
		...
		if (prueba==3) {
			if (solucion==1) {
				...
				especificacion="<span style='color: red'>La alternativa correcta es “oler” dado que se plantea determinar la principal función sensorial de la nariz.</span>";
	...
	function colocar_nro(id) {
		...
		if (nro_prueba==0) {
			...
			if (pregunta_actual_co==1)
				...
				texto=valor+": En este ejemplo, el apellido Tovar se ubica en la gaveta 37, ésta presenta el intervalo de letras entre la “TJ” y la “TZ”. Coloque 37 en la casilla. Presione siguiente";
