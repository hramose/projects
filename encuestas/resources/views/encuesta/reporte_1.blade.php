            ...
            <table id="tabla_reportes" class="display" cellspacing="0" width="100%" align="center">
				...
				<?php
					$sql = "select distinct(c.id) as id_candidato, c.nombres, 
						...
						r.fecha, r.perfil, r.nro_prueba, a.id_idioma, a.id as id_autorizacion, 
						a.id_usuario, r.id as id_resultado
					...
					foreach ($data as $data) {
						...
						if ($cedula=="")
							$cedula=0;
						$cedula=isset($data->cedula) ? number_format($cedula,0,".",".") : 0;
						?>
							<tr>								
								...
								<th>
									<div class="btn-group">									
										...
										<a target="new" href="<?=$ruta?>imprimir_reporte/<?=$id_candidato?>-1">
											<button class="btn btn-default" type="button">Imprimir Reporte</button>
										</a>										
										<?php
											if (!empty($data_alt))
												echo '
													<a target="new" href="'.$ruta.'imprimir_reporte/'.$id_candidato.'-2">
